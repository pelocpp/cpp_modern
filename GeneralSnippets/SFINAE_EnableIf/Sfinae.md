...
zu berücksichtigen ist. Das Problem bei Templates besteht darin, dass sie normalerweise zu umfassend sind.
Wenn sie mit Überladungen gemischt werden, kann das Ergebnis überraschend sein:
...
Welche der beiden Funktionen `foo` wird Ihrer Meinung nach beim Aufruf von `foo(123);` aufgerufen?
Die Antwort lautet: Es ist die Funktions Template Variante, die Ausgabe lautet folglich "`template 123`".
Der Grund dafür ist, dass ganzzahlige Literale standardmäßig signed sind
...
während die zweite perfekt übereinstimmt, sodass diese ausgewählt wird.
...
Dieser Code ist natürlich ungültig, da `int` keinen Member mit dem Namen `value_type` hat.
Man könnte also die Frage stellen, ob der Compiler in diesem Fall eine Fehlermeldung ausgeben sollte?
Er tut es nicht, denn in diesem Fall wäre das Schreiben von generischem Code in C ++ sehr schwierig.
...
was aber nicht einen Fehler nach sich zieht, der Compiler ignoriert diesen Kandidaten einfach und sieht die anderen an.
...
wird aufgrund des vorliegdenden Typs `T:value_type` im Rumpf der Funktion tatsächlich ein Kompilierungsfehler angezeigt!
Die Moral von der Geschichte ist: Wenn wir ein Template schreiben wollen, das nur für einige Typen sinnvoll ist,
dann müssen wir dafür Sorge leisten, dass der Mechanismus der "*Type Deduction*" bereits direkt in der Deklaration
