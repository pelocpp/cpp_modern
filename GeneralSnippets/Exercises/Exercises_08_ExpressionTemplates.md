...
wenn man diese in `std::vector<T>`-Objekten ablegt, so berechnen:
...
```
T product = 0;
product = product + a[0] * b[0];
product = product + a[1] * b[1];
product = product + a[2] * b[2];
product = product + a[3] * b[3];
product = product + a[4] * b[4];
```
...
vom Übersetzer erzeugen lassen! Erstellen Sie eine Klasseschablone `ScalarProduct`, die mit Hilfe von &ldquo;Loop Unrolling&rdquo;
